    ...
    class Hikvision
    {
        ...
        private void ImageCallbackFunc(IntPtr pData, ref MyCamera.MV_FRAME_OUT_INFO_EX pFrameInfo, IntPtr pUser)
        {
            try
            {
                ...
                //   // VImage img = new VImage("byte", pFrameInfo.nWidth, pFrameInfo.nHeight, pTemp);
                //   // img.GenImage1("byte", pFrameInfo.nWidth, pFrameInfo.nHeight, pTemp);
                //   // img.WriteImage("bmp", 0, "1.bmp");
                //}
        ...
        public Bitmap ReadImage()
        {
            ...
            temp = myCamera.MV_CC_GetOneFrameTimeout_NET(pData, nPayloadSize, ref stFrameInfo, 3000);//获取一帧图像，超时时间设置为1000
            if (MyCamera.MV_OK != temp)
            {
                return null;
            }
